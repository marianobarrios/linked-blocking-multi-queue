...
/**
 * An optionally-bounded blocking "multi-queue" based on linked nodes. A multi-queue is actually a
 * set of queues that are connected at the heads and have independent tails (the head of the queue
 * is that element that has been on the queue the longest time. The tail of the queue is that
 * element that has been on the queue the shortest time). New elements are added at the tail of one
 * of the queues, and the queue retrieval operations obtain elements from the head of some of the
 ...
 * {@code Queue} interfaces. The traditional queue interface is split in the traits: {@code
 * Offerable} and {@code Pollable}. Sub-queues do however implement Collection.
 *
...
public class LinkedBlockingMultiQueue<K, E> extends AbstractPollable<E> {
  ...
  /**
   * Represent a sub-queue inside a multi-queue. Instances of this class are just like any blocking
   * queue except that elements cannot be taken from their heads.
   */
