...
class BenchmarkLoopLbmq {
    ...
    Duration start() {
        ...
        Stream<LinkedBlockingMultiQueue<Integer, Object>.SubQueue> subQueues = IntStream.range(0, queues).mapToObj(
                i -> {
                    queue.addSubQueue(i, 1, individualQueueCapacity);
                    return queue.getSubQueue(i);
                });
        writers = subQueues.flatMap(subQueue ->
                IntStream.range(0, writerThreads).mapToObj(
                        i -> new Thread(() -> writer(subQueue), String.format("writer-%d", i)))
        ).collect(Collectors.toList());
        readers = IntStream.range(0, readerThreads).mapToObj(
                i -> new Thread(() -> reader(queue), String.format("reader-%d", i))
        ).collect(Collectors.toList());
    ...
    private void reader(Pollable<Object> pollable) {
        try {
            for (int i = 0; ; i++) {
                ...
                if (i % 5_000_000 == 0) {
                    System.out.printf("[%s] Read %d elements.\n", Thread.currentThread().getName(), i);
                }
