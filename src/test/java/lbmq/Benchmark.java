...
public class Benchmark {
  ...
  public static void main(String[] args) {
    if (warmup) {
      ...
      if (testAlt) {
        BenchmarkLoopAlt warmupAlt =
            new BenchmarkLoopAlt(
                warmupSize, totalQueueCapacity, warmupQueues, warmupWritersPerQueue, warmupReaders);
        warmupAlt.start();
      }
      if (testLbmq) {
        BenchmarkLoopLbmq warmupLbmq =
            new BenchmarkLoopLbmq(
                warmupSize, totalQueueCapacity, warmupQueues, warmupWritersPerQueue, warmupReaders);
        warmupLbmq.start();
      }
    }
    ...
    if (testLbmq) {
      BenchmarkLoopLbmq testLbmq =
          new BenchmarkLoopLbmq(
              benchmarkSize, totalQueueCapacity, queues, writersPerQueue, readers);
