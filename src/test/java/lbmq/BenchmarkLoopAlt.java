...
class BenchmarkLoopAlt {
    ...
    Duration start() {
        ...
        Stream<LinkedBlockingQueue<Object>> subQueues = IntStream.range(0, queues).mapToObj(i ->
                new LinkedBlockingQueue<>(individualQueueCapacity)
        );
        writers = subQueues.flatMap(subQueue ->
                IntStream.range(0, writerThreadsPerQueue).mapToObj(i ->
                        new Thread(() -> writer(subQueue, notifier), String.format("writer-%d", i))
                )
        ).collect(Collectors.toList());
        readers = IntStream.range(0, readerThreads).mapToObj(i ->
            new Thread(() -> reader(notifier), String.format("reader-%d", i))
        ).collect(Collectors.toList());
    ...
    private void writer(BlockingQueue<Object> offerable, LinkedBlockingQueue<BlockingQueue<Object>> notifier) {
        try {
            for (int i = 0; ; i++) {
                ...
                if (i % 5_000_000 == 0) {
                    System.out.printf("[%s] Wrote %d elements. Queue size: %d\n", Thread.currentThread().getName(), i,
                            offerable.size());
                }
            }
    ...
    private void reader(BlockingQueue<BlockingQueue<Object>> notifier) {
        ...
        try {
            for (int i = 0; ; i++) {
                ...
                if (i % 5_000_000 == 0) {
                    System.out.printf("[%s] Read %d elements. Queue size: %d\n", Thread.currentThread().getName(), i,
                            queueSize);
                }
