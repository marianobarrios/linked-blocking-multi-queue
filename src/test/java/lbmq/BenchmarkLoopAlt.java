...
class BenchmarkLoopAlt {
  ...
  Duration start() {
    LinkedBlockingQueue<BlockingQueue<Object>> notifier =
        new LinkedBlockingQueue<>(individualQueueCapacity);
    Stream<LinkedBlockingQueue<Object>> subQueues =
        IntStream.range(0, queues)
            .mapToObj(i -> new LinkedBlockingQueue<>(individualQueueCapacity));
    writers =
        subQueues
            .flatMap(
                subQueue ->
                    IntStream.range(0, writerThreadsPerQueue)
                        .mapToObj(
                            i ->
                                new Thread(
                                    () -> writer(subQueue, notifier),
                                    String.format("writer-%d", i))))
  ...
  private void writer(
      BlockingQueue<Object> offerable, LinkedBlockingQueue<BlockingQueue<Object>> notifier) {
    try {
      for (int i = 0; ; i++) {
